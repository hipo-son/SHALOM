...
class QEBackend:
    ...
    def _write_pw_input(
        ...
        # --- QE 7.1+ Hubbard syntax migration ---
        # Convert old lda_plus_u / Hubbard_U(i) in &SYSTEM to HUBBARD card.
        hubbard_entries: List[str] = []
        sys_ns = namelists.get("SYSTEM", {})
        if sys_ns.pop("lda_plus_u", None):
            # Map L quantum number to orbital label (with principal quantum number)
            # 3d TMs: n=3, 4f lanthanides: n=4, 5f actinides: n=5
            _l_to_orbital = {0: "1s", 1: "2p", 2: "3d", 3: "4f"}
            # Collect Hubbard_U(i) entries, keyed by species index
            hub_keys = [k for k in list(sys_ns.keys())
                        if k.lower().startswith("hubbard_u(")]
            for hk in hub_keys:
                u_val = sys_ns.pop(hk)
                # Extract species index (1-based)
                idx = int(re.search(r"\((\d+)\)", hk).group(1))
                if idx <= len(species_order):
                    el = species_order[idx - 1]
                    # Look up orbital from HUBBARD_U_VALUES
                    from shalom.backends._physics import HUBBARD_U_VALUES
                    l_qn = HUBBARD_U_VALUES.get(el, (2, 0, 0))[0]
                    orb = _l_to_orbital.get(l_qn, "3d")
                    hubbard_entries.append(f"U {el}-{orb} {u_val}")
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            ...
            # HUBBARD card (QE 7.1+ syntax, must follow ATOMIC_SPECIES)
            if hubbard_entries:
                f.write("HUBBARD (ortho-atomic)\n")
                for entry in hubbard_entries:
                    f.write(f"  {entry}\n")
                f.write("\n")
